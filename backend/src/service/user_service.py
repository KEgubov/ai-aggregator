...
class UserService:
    ...
    async def user_validate(
        self, session: AsyncSession, user: UserAddDTO
    ) -> UserDTO | None:
        ...
        added_user = await self.user_repository.create_user_in_db(session, model_user)
        if added_user:
    ...
    async def validate_current_user(
        ...
        current_user = await self.user_repository.get_current_user_by_id(
            session, user_id
        )
        if current_user:
            result_dto = CurrentUserDTO.model_validate(
    ...
    async def get_user_profile(
        ...
        profile = await self.user_repository.get_current_user_by_id(session, user_id)
        if profile:
