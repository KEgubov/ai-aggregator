...
Проект в разработке. Создан для портфолио и демонстрирует backend и frontend: аутентификация, проектирование API, работа с БД, оркестрация нескольких LLM-провайдеров, Docker.

Все данные хранятся в локальном PostgreSQL на машине разработчика. При запуске через Docker сайт не покажет никаких данных — в контейнерах нет вашей локальной БД с пользователями, чатами и сообщениями.
...
- Приглашение других пользователей в чат по инвайт-ссылке (ссылка копируется из чата, получатель вступает после входа)
